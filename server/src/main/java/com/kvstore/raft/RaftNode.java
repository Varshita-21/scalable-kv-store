...
public class RaftNode {
    ...
    public RaftNode(String nodeId, List<String> peerIds, SQLiteStorage storage) {
        ...
        peerIds.forEach(id -> {
            if(!id.equals(nodeId)) {
                this.peers.put(id, new NodeInfo(id));
            }
        });
        ...
        initializeGrpcChannels(peerIds);  // Pass peerIds here
    }
...
private void initializeGrpcChannels(List<String> peerIds) {
        logger.info("initialize Grpc Channels:");
    for (String peerId : peerIds) {
        retryConnection(peerId, 5);
        if (!peerId.equals(nodeId)) {  // Don't create a channel for self
            NodeInfo peerInfo = peers.get(peerId);
            ManagedChannel channel = ManagedChannelBuilder.forAddress(peerInfo.getHost(), peerInfo.getPort())
                    .usePlaintext()
                    .build();
            grpcChannels.put(peerId, channel);
            logger.info("Initialized gRPC channel for peer: {}, address: {}:{}",
                    peerId, peerInfo.getHost(), peerInfo.getPort());
        }
    }
}

    private void retryConnection(String peerId, int maxRetries) {
        for (int i = 0; i < maxRetries; i++) {
            try {
                NodeInfo peerInfo = new NodeInfo(peerId);
                ManagedChannel channel = ManagedChannelBuilder.forAddress(peerInfo.getHost(), peerInfo.getPort())
                        .usePlaintext()
                        .build();
                grpcChannels.put(peerId, channel);
                ...
                logger.warn("Failed to connect to peer: {}. Retry {}/{}", peerId, i+1, maxRetries);
                try {
        ...
        logger.error("Failed to connect to peer: {} after {} retries", peerId, maxRetries);
    }
    ...
    public void start(){
       try {
           ...
           System.out.println("end waiting for start");
       } catch (Exception e){
...
//        if (state.getCurrentRole() != RaftState.Role.LEADER) {
//            logger.warn("Not leader, cannot propose entry");
...
public CompletableFuture<Boolean> proposeEntry(String key, String value) {
    ...
    logger.info("Proposing entry: {}", entry);

    long index = storage.appendLogEntry(entry);
    logger.info("Entry appended to log at index: {}", index);

    return replicateEntry(index)
            .thenApply(success -> {
                logger.info("Entry replication result: {}", success);
                if (success) {
                    updateCommitIndex();
                    applyCommittedEntries();
                }
